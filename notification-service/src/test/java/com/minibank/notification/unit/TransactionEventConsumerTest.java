...
class TransactionEventConsumerTest {
    ...
    class RedisIdempotencyTests {
        ...
        void consumeEvent_FirstTime_ProcessesAndSetsRedisKey() {
            ...
            verify(valueOperations).set(eq(expectedRedisKey), eq("1"), durationCaptor.capture());
            assertEquals(24, durationCaptor.getValue().toHours(),
                    "Idempotency TTL must be 24 hours");
        ...
        void consumeEvent_RedisHit_SkipsProcessing() {
            ...
            // Assert — Message acknowledged (processed successfully — just skipped)
            verify(acknowledgment).acknowledge();
        }
        ...
        @Test
        @DisplayName("Redis SET NX is called with correct key format")
        void consumeEvent_RedisKeyFormat_IsCorrect() {
            ...
            // Assert — key format: notification:processed:{eventId}
    ...
    class DbDoubleCheckTests {
        ...
        void consumeEvent_RedisMiss_DbHitSender_SkipsProcessing() {
            ...
            verify(notificationRepository).existsByIdempotencyKey("tx-" + testEventId + "-sender");
            // No need to check receiver if sender already hit
            verify(notificationService, never()).createFromTransactionEvent(any());
            verify(acknowledgment).acknowledge();
        }
    ...
    class ExceptionRollbackTests {
        ...
        void consumeEvent_ExceptionDuringProcessing_DeletesRedisKey() {
            ...
            // Assert — Message still acknowledged (avoid infinite retry loop)
            verify(acknowledgment).acknowledge();
        }
